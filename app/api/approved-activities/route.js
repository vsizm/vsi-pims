...
function normalizeActivity(a) {
  ...
  const programmeCode = String(a.programmeCode ?? a.programCode ?? '').trim().toUpperCase();
  const existingProgramme = String(a.programme ?? a.programmeName ?? '').trim();
  const activityPrefix = activityCode.split('-')[0].toUpperCase();
  const mappedProgramme = programmeByCode[programmeCode] ?? programmeByCode[activityPrefix];
  ...
  return {
    ...
    programme: mappedProgramme ?? existingProgramme,
