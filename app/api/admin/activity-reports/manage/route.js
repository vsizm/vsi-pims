...
async function prepare(sql) { await sql`ALTER TABLE activity_reports ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ`; await sql`ALTER TABLE activity_reports ADD COLUMN IF NOT EXISTS deleted_by TEXT`; await sql`ALTER TABLE activity_reports ADD COLUMN IF NOT EXISTS last_edited_by TEXT`; await sql`ALTER TABLE activity_reports ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ`; await sql`ALTER TABLE activity_reports ADD COLUMN IF NOT EXISTS review_status TEXT NOT NULL DEFAULT 'PENDING_REVIEW'`; await sql`ALTER TABLE activity_reports ADD COLUMN IF NOT EXISTS reviewed_by TEXT`; await sql`ALTER TABLE activity_reports ADD COLUMN IF NOT EXISTS reviewed_at TIMESTAMPTZ`; await sql`ALTER TABLE activity_reports ADD COLUMN IF NOT EXISTS review_comment TEXT`; await sql`CREATE TABLE IF NOT EXISTS activity_report_audit (id BIGSERIAL PRIMARY KEY, reference TEXT NOT NULL, action TEXT NOT NULL, actor TEXT NOT NULL, details TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`; }
...
export async function POST(request) {
  ...
  try { const sql = neon(process.env.DATABASE_URL); await prepare(sql); await sql`UPDATE activity_reports SET review_status = ${status}, reviewed_by = ${actor}, reviewed_at = NOW(), review_comment = ${comment || null}, updated_at = NOW() WHERE reference = ${reference} AND deleted_at IS NULL`; await sql`INSERT INTO activity_report_audit (reference, action, actor, details) VALUES (${reference}, ${status}, ${actor}, ${comment || null})`; return Response.json({ ok: true, status }); } catch (error) { console.error('activity report review failed', error); return Response.json({ error: 'Unable to save review decision.' }, { status: 500 }); }
}
...
export async function DELETE(request) {
  ...
  try { const sql = neon(process.env.DATABASE_URL); await prepare(sql); const result = await sql`UPDATE activity_reports SET deleted_at = NOW(), deleted_by = ${actor}, updated_at = NOW() WHERE reference = ${reference} AND deleted_at IS NULL`; if (!result.count) return Response.json({ error: 'Report not found or already deleted.' }, { status: 404 }); await sql`INSERT INTO activity_report_audit (reference, action, actor) VALUES (${reference}, 'DELETE', ${actor})`; return Response.json({ ok: true }); } catch (error) { console.error('activity report delete failed', error); return Response.json({ error: 'Unable to delete report.' }, { status: 500 }); }
}
