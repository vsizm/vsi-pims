...
export async function PATCH(request) {
  ...
  const { reference, fields = {}, actor = 'Administrator' } = body || {};
  ...
  try {
    ...
    for (const key of allowed) if (Object.prototype.hasOwnProperty.call(fields, key)) await sql`UPDATE activity_reports SET ${sql(key)} = ${fields[key]}, updated_at = NOW() WHERE reference = ${reference} AND deleted_at IS NULL`;
  ...
  } catch (error) { console.error(error); return Response.json({ error: 'Unable to save changes.' }, { status: 500 }); }
}

export async function DELETE(request) {
  if (!(await guard())) return Response.json({ error: 'Unauthorised.' }, { status: 401 });
  const body = await request.json();
  const { reference, actor = 'Administrator' } = body || {};
  ...
  try {
    ...
    await sql`UPDATE activity_reports SET deleted_at = NOW(), deleted_by = ${actor}, updated_at = NOW() WHERE reference = ${reference} AND deleted_at IS NULL`;
    await sql`INSERT INTO activity_report_audit (reference, action, actor) VALUES (${reference}, 'DELETE', ${actor})`;
    return Response.json({ ok: true });
  } catch (error) { console.error(error); return Response.json({ error: 'Unable to delete report.' }, { status: 500 }); }
}
