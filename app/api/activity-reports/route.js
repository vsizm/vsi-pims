...
const participantFields = [
  ...
  'reachedMaleChildren','reachedMaleYouth','reachedMaleAdult','reachedFemaleChildren','reachedFemaleAdult',
...
async function sendReportEmail({ recipient, reference, activityTitle, reporterName }) {
  ...
  if (!apiKey || !recipient) {
    console.warn('RESEND_API_KEY or recipient is not configured; report email was not sent');
    return;
  }
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      html: `<p>Dear ${reporterName || 'VSI Officer'},</p><p>Your activity report has been successfully submitted to <strong>VSI PIMS</strong>.</p><p><strong>Reference:</strong> ${reference}<br><strong>Activity:</strong> ${activityTitle || 'Activity report'}</p><p>The report is now recorded for review. Please keep the reference number for your records.</p><p>VSI PIMS<br>Visionary Students Initiative</p>`,
    }),
  });
...
export async function POST(request) {
  try {
    ...
    // Test mode: validate email only when an email has actually been supplied.
    ...
    if (Object.values(participantData).some((n) => n < 0 || !Number.isFinite(n))) {
      return Response.json({ error: 'Participant numbers must be valid non-negative numbers.' }, { status: 400 });
    }
    ...
    if (participantPerformance !== 'ACHIEVED' && participantPerformance !== '' && !String(body.participantPerformanceJustification || '').trim()) {
      return Response.json({ error: 'Please provide the justification for the participant performance result.' }, { status: 400 });
    }
    if (plannedTotal < 0 || reachedTotal < 0 || !Number.isFinite(plannedTotal) || !Number.isFinite(reachedTotal)) {
      return Response.json({ error: 'Participant totals must be valid non-negative numbers.' }, { status: 400 });
    }
    ...
    attachments.push({
      category: 'participant-breakdown',
      plannedTotal,
      reachedTotal,
      performance: participantPerformance,
      achievementPercent: participantPerformancePercent,
      variance: reachedTotal - plannedTotal,
      justification: body.participantPerformanceJustification || null,
      breakdown: participantData,
    });
