...
export async function POST(request) {
  ...
  try {
    ...
    // The browser only displays these catalogue fields as read-only. Re-resolve them here
    // from the authoritative PMIS catalogue so a crafted request cannot change programme,
    // project, directorate, SDG, or AU Agenda 2063 metadata.
    const sql = neon(process.env.DATABASE_URL);
    const activityCode = String(body.activityCode || '').trim();
    if (!activityCode) return Response.json({ error: 'Please select an approved VSI activity.' }, { status: 400 });
    const catalogueRows = await sql`SELECT a.code, a.name, p.name AS programme, pr.name AS project, a.directorate, a.un_sdgs_alignment AS "unSdgsAlignment", a.au_agenda_2063_alignment AS "auAgenda2063Alignment" FROM activities a JOIN projects pr ON pr.code = a.project_code JOIN programmes p ON p.code = pr.programme_code WHERE a.code = ${activityCode} AND a.approval_status = 'APPROVED' LIMIT 1`;
    const approvedActivity = catalogueRows[0];
    if (!approvedActivity) return Response.json({ error: 'The selected activity is not an approved activity in the PMIS catalogue.' }, { status: 400 });
    ...
    const pwdMale = participantData.reachedPwdMaleChildren + participantData.reachedPwdMaleYouth + participantData.reachedPwdMaleAdult;
    const pwdFemale = participantData.reachedPwdFemaleChildren + participantData.reachedPwdFemaleYouth + participantData.reachedPwdFemaleAdult;
    const other = 0;
