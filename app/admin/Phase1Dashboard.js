...
export default function Phase1Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const followUp = concernsList.filter(s => /requires follow[- ]?up/i.test(s)).length;
    return { total: reports.length, pending, approved, returned, rejected, planned, reached, budget, spent, assessment, identified, followUp, health: reports.length ? Math.round(approved / reports.length * 100) : 0, achievement: pct(reached, planned) };
  }, [reports, details]);
  ...
  return <div className="phase1-app">
    ...
    <main className="phase1-main">
      <header className="phase1-header">
        <div><div className="phase1-kicker">VSI ADMINISTRATION · MANAGEMENT INTELLIGENCE</div><h1>Administration Dashboard</h1><p>Executive overview of reporting performance, reach, assessment and safeguarding signals.</p></div>
        <Link href="/admin/reports" className="phase1-action">Reports Inbox →</Link>
      </header>
      ...
      {!loading && !error && <>
        ...
        <section className="phase1-grid">
          <article className="phase1-card wide">
            <div className="phase1-card-head"><div><span>ACTIVITY REPORT INTELLIGENCE</span><h2>Organisation-wide performance</h2></div><Link href="/admin/reports">Open register →</Link></div>
            <div className="phase1-metrics">
              <div><small>Approved reach</small><strong>{stats.reached.toLocaleString()}</strong><em>participants</em></div>
              <div><small>Target achievement</small><strong>{stats.planned ? `${stats.achievement}%` : '—'}</strong><em>{stats.planned ? `${stats.reached.toLocaleString()} of ${stats.planned.toLocaleString()}` : 'No target data'}</em></div>
              <div><small>Returned</small><strong>{stats.returned}</strong><em>require correction</em></div>
              <div><small>Rejected</small><strong>{stats.rejected}</strong><em>not accepted</em></div>
            </div>
          </article>
          ...
          <article className="phase1-card">
            <div className="phase1-card-head"><div><span>OVERALL ASSESSMENT</span><h2>Operational effectiveness</h2></div></div>
            <div className="assessment"><div className="assessment-score">{stats.assessment ? `${stats.assessment}%` : '—'}</div><div className="assessment-track"><i style={{width:`${stats.assessment}%`}} /></div><p>Calculated from assessment information available in approved Section 12 reports.</p></div>
          </article>
          ...
          <article className="phase1-card">
            <div className="phase1-card-head"><div><span>REACH & TARGETS</span><h2>Approved activity reach</h2></div></div>
            <div className="reach-value"><strong>{stats.reached.toLocaleString()}</strong><span>participants reached</span></div>
            <div className="reach-track"><i style={{width:`${Math.min(100,stats.achievement)}%`}} /></div>
            <div className="reach-label"><span>{stats.planned ? `${stats.achievement}% of approved target` : 'Target data not yet available'}</span></div>
          </article>
