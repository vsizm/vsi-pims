...
const statusLabel = { PENDING_REVIEW: 'PENDING REVIEW', APPROVED: 'APPROVED', RETURNED: 'RETURNED', REJECTED: 'REJECTED' };

function Card({ label, value, note, accent = 'blue' }) {
...
function Sidebar({ active }) {
  return <aside className="admin-sidebar">
    ...
    <nav>{NAV.map(([label, href]) => <Link key={label} href={href} className={active === label ? 'active' : ''}><span className="nav-dot" />{label}</Link>)}</nav>
...
export default function AdminDashboard({ active = 'Dashboard', title = 'Administration Dashboard', subtitle = 'A clear management view of VSI activity reporting, finance and MEAL performance.' }) {
  ...
  const byProgramme = useMemo(() => {
    const map = new Map();
    details.forEach(r => { const key = r.programme || 'Unspecified'; const row = map.get(key) || { name: key, reports: 0, participants: 0, budget: 0, spent: 0 }; row.reports += 1; row.participants += Number(r.participant_total || 0); row.budget += Number(r.approved_budget || 0); row.spent += Number(r.actual_spent || 0); map.set(key, row); });
